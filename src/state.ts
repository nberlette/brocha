...
export class State {
  ...
  decompress(): Uint8Array {
    ...
    while (true) {
      const chunk = new Int8Array(CHUNK_SIZE);
      this.chunks.push(chunk);
      this.output = chunk;
      this.outputOffset = 0;
      this.outputLength = CHUNK_SIZE;
      this.outputUsed = 0;
      this.decompressChunk();
      this.totalOutput += this.outputUsed;
      if (this.outputUsed < CHUNK_SIZE) break;
    }
    this.close();
    const result = new Int8Array(this.totalOutput);
    for (let offset = 0, i = 0; i < this.chunks.length; ++i) {
      const chunk = this.chunks[i];
      let end = offset + CHUNK_SIZE;
      if (end > this.totalOutput) end = this.totalOutput;
  ...
  readInput(data: Int8Array, offset: number, length: number): number {
    ...
    let end = src.offset + length;
    if (end > src.data.length) end = src.data.length;
    const bytesRead = end - src.offset;
    data.set(src.data.subarray(src.offset, end), offset);
  ...
  protected init(): this {
    if (this.runningState !== 0) {
      throw new BrotliDecoderUnreachableError("State must be uninitialized");
    }
    this.blockTrees = new Int32Array(3091);
    this.blockTrees[0] = 7;
    this.distRbIdx = 3;
    const maxDistanceAlphabetLimit = this.calculateDistanceAlphabetLimit(
      0x7FFFFFFC,
      3,
      120,
    );
    this.distExtraBits = new Int8Array(maxDistanceAlphabetLimit);
    this.distOffset = new Int32Array(maxDistanceAlphabetLimit);
    this.initBitReader();
    this.runningState = 1;
    return this;
  }
  ...
  protected initBitReader(): this {
    this.byteBuffer = new Int8Array(4160);
    this.accumulator32 = 0;
    this.shortBuffer = new Int16Array(2080);
    this.bitOffset = 32;
    this.halfOffset = 2048;
  ...
  protected close(): void {
    if (this.runningState === 0) {
      throw new BrotliDecoderUnreachableError("State must be initialized");
    }
    if (this.runningState !== 11) {
      this.runningState = 11;
      this.input.data = new Int8Array(0);
      this.input.offset = 0;
    }
  }

  protected prepare(): this {
    if (this.halfOffset > 2030) this.doReadMoreInput();
    this.checkHealth(0);
    this.accumulator32 = (this.shortBuffer[this.halfOffset++] << 16) |
      (this.accumulator32 >>> 16);
    this.bitOffset -= 16;
    this.accumulator32 = (this.shortBuffer[this.halfOffset++] << 16) |
      (this.accumulator32 >>> 16);
    this.bitOffset -= 16;
  ...
  protected reload(): this {
    if (this.bitOffset === 32) this.prepare();
    return this;
  }

  protected attachDictionaryChunk(buf: string | BufferSource): void {
    if (this.runningState !== 1) {
      throw new BrotliDecoderUnreachableError(
        "State must be freshly initialized",
      );
    }
    if (this.cdNumChunks === 0) {
      this.cdChunks = new Array(16);
      this.cdChunkOffsets = new Int32Array(16);
      this.cdBlockBits = -1;
    }
    if (this.cdNumChunks === 15) {
      throw new BrotliDecoderDictionaryError("Too many dictionary chunks");
    }
    let data: Int8Array;
    if (typeof buf === "string") {
      data = toUsAsciiBytes(buf);
    } else if (isTypedArray(buf, "Int8Array")) {
      data = buf;
    } else if (ArrayBufferIsView(buf)) {
      ...
      try {
        ...
        try {
          SharedArrayBufferPrototypeGetByteLength(buf);
          data = new Int8Array(buf);
        } catch (_) {
          throw new BrotliDecoderInvalidArgumentsError(
            "Invalid dictionary buffer type",
          );
        }
      }
    }
    this.cdChunks[this.cdNumChunks] = data;
    this.cdNumChunks++;
    this.cdTotalSize += data.length;
    this.cdChunkOffsets[this.cdNumChunks] = this.cdTotalSize;
  }

  protected initializeCompoundDictionary(): void {
    this.cdBlockMap = new Int8Array(256);
    let blockBits = 8;
    while (((this.cdTotalSize - 1) >>> blockBits) !== 0) blockBits++;
    blockBits -= 8;
    this.cdBlockBits = blockBits;
    let cursor = 0, index = 0;
    while (cursor < this.cdTotalSize) {
      while (this.cdChunkOffsets[index + 1] < cursor) index++;
      this.cdBlockMap[cursor >>> blockBits] = index;
      cursor += 1 << blockBits;
    }
  }
  ...
  protected initializeCompoundDictionaryCopy(
    ...
    if (this.cdBlockBits === -1) this.initializeCompoundDictionary();
    let index = this.cdBlockMap[address >>> this.cdBlockBits];
    while (address >= this.cdChunkOffsets[index + 1]) index++;
    if (this.cdTotalSize > address + length) {
      throw new BrotliDecoderDictionaryError(
        "Compound dictionary copy out of bounds",
      );
    }
    this.distRbIdx = (this.distRbIdx + 1) & 0x3;
    this.rings[this.distRbIdx] = this.distance;
    this.metaBlockLength -= length;
    this.cdBrIndex = index;
    this.cdBrOffset = address - this.cdChunkOffsets[index];
    this.cdBrLength = length;
    this.cdBrCopied = 0;
  }
  ...
  protected copyFromCompoundDictionary(fence: number): number {
    let pos = this.pos;
    const origPos = pos;
    while (this.cdBrLength !== this.cdBrCopied) {
      const space = fence - pos;
      const chunkLength = this.cdChunkOffsets[this.cdBrIndex + 1] -
        this.cdChunkOffsets[this.cdBrIndex];
      const remChunkLength = chunkLength - this.cdBrOffset;
      let length = this.cdBrLength - this.cdBrCopied;
      if (length > remChunkLength) length = remChunkLength;
      if (length > space) length = space;
      this.ringBuffer.set(
        this.cdChunks[this.cdBrIndex].slice(
          this.cdBrOffset,
          this.cdBrOffset + length,
        ),
        pos,
      );
      pos += length;
      this.cdBrOffset += length;
      this.cdBrCopied += length;
      if (length === remChunkLength) this.cdBrIndex++, this.cdBrOffset = 0;
      if (pos >= fence) break;
    }
    return pos - origPos;
  }

  protected copyRawBytes(
    data: Int8Array,
    offset: number,
    length: number,
  ): void {
    let pos = offset, len = length;
    if ((this.bitOffset & 7) !== 0) {
      throw new BrotliDecoderAllocRingBuffer1Error("Unaligned copy offset");
    }
    while ((this.bitOffset !== 32) && (len !== 0)) {
      data[pos++] = this.accumulator32 >>> this.bitOffset;
      this.bitOffset += 8;
      len--;
    }
    if (len === 0) return;
    let copyNibbles = this.halfAvailable();
    if (copyNibbles > (len >> 1)) copyNibbles = len >> 1;
    if (copyNibbles > 0) {
      const readOffset = this.halfOffset << 1;
      const delta = copyNibbles << 1;
      const chunk = this.byteBuffer.subarray(readOffset, readOffset + delta);
      data.set(chunk, pos);
      pos += delta;
      len -= delta;
      this.halfOffset += copyNibbles;
    }
    if (len === 0) return;
    if (this.halfAvailable() > 0) {
      if (this.bitOffset >= 16) {
        this.accumulator32 = (this.shortBuffer[this.halfOffset++] << 16) |
          (this.accumulator32 >>> 16);
        this.bitOffset -= 16;
      }
    ...
    while (len > 0) {
      const chunkLen = this.readInput(data, pos, len);
      if (chunkLen === -1) {
        throw new BrotliDecoderBlockLength2Error("Unexpected end of input");
      }
      pos += chunkLen;
      len -= chunkLen;
    }
  ...
  protected copyUncompressedData(): void {
    ...
    let chunkLength = this.ringBufferSize - this.pos;
    if (chunkLength > this.metaBlockLength) chunkLength = this.metaBlockLength;
    this.copyRawBytes(ringBuffer, this.pos, chunkLength);
    this.metaBlockLength -= chunkLength;
    this.pos += chunkLength;
    if (this.pos === this.ringBufferSize) {
      this.nextRunningState = 6;
      this.runningState = 12;
      return;
    }
    this.reload();
    this.runningState = 2;
  ...
  protected bytesToNibbles(byteLen: number): void {
    ...
    const shortBuffer = this.shortBuffer;
    for (let i = 0; i < byteLen >> 1; ++i) {
      shortBuffer[i] = (byteBuffer[i * 2] & 0xFF) |
        ((byteBuffer[(i * 2) + 1] & 0xFF) << 8);
    }
  }

  protected calculateDistanceLut(alphabetSizeLimit: number): void {
    const distExtraBits = this.distExtraBits;
    const distOffset = this.distOffset;
    const npostfix = this.distancePostfixBits;
    const ndirect = this.numDirectDistanceCodes;
    const postfix = 1 << npostfix;
    let bits = 1, half = 0, i = 16;
  ...
  protected calculateFence(): number {
    let result = this.ringBufferSize;
    if (this.isEager !== 0) {
      const offset = this.outputLength - this.outputUsed;
      const next = this.ringBufferBytesWritten + offset;
  ...
  protected decompressChunk(): void {
    if (!State.initialized || this.runningState === 0) {
      throw new BrotliDecoderDictionaryNotSetError(
        "Dictionary not initialized",
      );
    }
    if (this.runningState === 11) {
      throw new BrotliDecoderUnreachableError("Cannot decompress after close");
    }
    if (this.runningState === 1) {
      const windowBits = this.decodeWindowBits();
      if (windowBits === -1) {
        throw new BrotliDecoderWindowBitsError("Invalid 'windowBits' code");
      }
      this.maxRingBufferSize = 1 << windowBits;
      this.maxBackwardDistance = this.maxRingBufferSize - 16;
      this.runningState = 2;
    }
    // let fence = this.calculateFence();
    let fence = this.ringBufferSize;
    // inlined calculateFence() here
    if (this.isEager !== 0) {
      const offset = this.outputLength - this.outputUsed;
      const next = this.ringBufferBytesWritten + offset;
      if (next < fence) fence = next;
    }
    let ringBufferMask = this.ringBufferSize - 1;
    let ringBuffer = this.ringBuffer;
    while (this.runningState !== 10) {
      switch (this.runningState) {
        case 2:
          if (this.metaBlockLength < 0) {
            throw new BrotliDecoderBlockLength1Error("Invalid block length");
          }
          // this.readNextMetablockHeader();

          // begin inlined readNextMetablockHeader()
          this.readNextMetablockHeader();
          // end inlined readNextMetablockHeader()

          // fence = this.calculateFence();
          // inlined calculateFence() here
        ...
        case 4: {
          ...
          if (this.commandBlockLength === 0) {
            this.commandBlockLength = this.decodeBlockTypeAndLength(
              1,
          ...
          const cmdCode = this.readSymbol(
            this.commandTreeGroup,
            this.commandTreeIdx,
          ) << 2;
          const insertAndCopyExtraBits = COMMAND_LOOKUP_TABLE[cmdCode];
          const insertLengthOffset = COMMAND_LOOKUP_TABLE[cmdCode + 1];
          const copyLengthOffset = COMMAND_LOOKUP_TABLE[cmdCode + 2];
          this.distanceCode = COMMAND_LOOKUP_TABLE[cmdCode + 3];
          if (this.bitOffset >= 16) {
            this.accumulator32 = (this.shortBuffer[this.halfOffset++] << 16) |
              (this.accumulator32 >>> 16);
            this.bitOffset -= 16;
          ...
          this.insertLength = insertLengthOffset +
            ((insertLengthExtraBits <= 16)
              ? this.readFewBits(insertLengthExtraBits)
              : this.readManyBits(insertLengthExtraBits));
          if (this.bitOffset >= 16) {
            this.accumulator32 = (this.shortBuffer[this.halfOffset++] << 16) |
              (this.accumulator32 >>> 16);
            this.bitOffset -= 16;
          ...
          this.copyLength = copyLengthOffset +
            ((copyLengthExtraBits <= 16)
              ? this.readFewBits(copyLengthExtraBits)
              : this.readManyBits(copyLengthExtraBits));
          this.j = 0;
          this.runningState = 7;
        }
        ...
        case 7:
          if (this.trivialLiteralContext !== 0) {
            while (this.j < this.insertLength) {
              if (this.halfOffset > 2030) this.doReadMoreInput();
              if (!this.literalBlockLength) this.decodeLiteralBlockSwitch();
              this.literalBlockLength--;
              if (this.bitOffset >= 16) {
                this.accumulator32 =
                  (this.shortBuffer[this.halfOffset++] << 16) |
                  (this.accumulator32 >>> 16);
                this.bitOffset -= 16;
              }
              ringBuffer[this.pos] = this.readSymbol(
              ...
              this.j++;
              if (this.pos >= fence) {
                this.nextRunningState = 7;
                this.runningState = 12;
                break;
              }
            }
          } else {
            let prev1 = ringBuffer[(this.pos - 1) & ringBufferMask] & 0xFF;
            let prev2 = ringBuffer[(this.pos - 2) & ringBufferMask] & 0xFF;
            while (this.j < this.insertLength) {
              if (this.halfOffset > 2030) this.doReadMoreInput();
              if (!this.literalBlockLength) this.decodeLiteralBlockSwitch();
              const literalContext = LOOKUP[this.contextLookupOffset1 + prev1] |
                LOOKUP[this.contextLookupOffset2 + prev2];
              const literalTreeIdx =
                this.contextMap[this.contextMapSlice + literalContext] & 0xFF;
              this.literalBlockLength--;
              prev2 = prev1;
              if (this.bitOffset >= 16) {
                this.accumulator32 =
              ...
              prev1 = this.readSymbol(this.literalTreeGroup, literalTreeIdx);
              ringBuffer[this.pos++] = prev1;
              this.j++;
              if (this.pos >= fence) {
                this.nextRunningState = 7;
                this.runningState = 12;
          ...
          if (this.runningState !== 7) continue;
          this.metaBlockLength -= this.insertLength;
          if (this.metaBlockLength <= 0) {
            this.runningState = 4;
            continue;
          }
          let dsc = this.distanceCode;
          if (dsc < 0) {
            this.distance = this.rings[this.distRbIdx];
          } else {
            if (this.halfOffset > 2030) this.doReadMoreInput();
            if (this.distanceBlockLength === 0) {
              this.distanceBlockLength = this.decodeBlockTypeAndLength(
                2,
                this.numDistanceBlockTypes,
              );
              this.distContextMapSlice = this.rings[9] << 2;
            }
            ...
            const slice = this.distContextMapSlice + dsc;
            const distTreeIdx = this.distContextMap[slice] & 0xFF;
            dsc = this.readSymbol(this.distanceTreeGroup, distTreeIdx);
            if (dsc < 16) {
              const index = (this.distRbIdx + DSC_INDEX_OFFSET[dsc]) & 0x3;
              this.distance = this.rings[index] + DSC_VALUE_OFFSET[dsc];
              if (this.distance < 0) {
                throw new BrotliDecoderDistanceError("Negative distance");
              }
            } else {
              const extraBits = this.distExtraBits[dsc];
              let bits: number;
              if (this.bitOffset + extraBits <= 32) {
                ...
                bits = (extraBits <= 16)
                  ? this.readFewBits(extraBits)
                  : this.readManyBits(extraBits);
              }
              this.distance = this.distOffset[dsc] +
                (bits << this.distancePostfixBits);
            }
          }
          this.maxDistance = (
              this.maxDistance !== this.maxBackwardDistance &&
              this.pos < this.maxBackwardDistance
            )
            ? this.pos
            : this.maxBackwardDistance;
          ...
          if (dsc > 0) {
            this.distRbIdx += 1;
            this.rings[this.distRbIdx &= 0x3] = this.distance;
          }
          if (this.copyLength > this.metaBlockLength) {
            throw new BrotliDecoderBlockLength2Error("Invalid copy length");
          }
          this.j = 0;
          this.runningState = 8;
        // deno-lint-ignore no-case-declarations
        case 8:
          let dst = this.pos, src = (dst - this.distance) & ringBufferMask;
          const copyLength = this.copyLength - this.j;
          const srcEnd = src + copyLength, dstEnd = dst + copyLength;
          if ((srcEnd < ringBufferMask) && (dstEnd < ringBufferMask)) {
            ...
            while (this.j < this.copyLength) {
              ringBuffer[this.pos] =
                ringBuffer[(this.pos - this.distance) & ringBufferMask];
              this.metaBlockLength--;
              this.pos++;
              this.j++;
              if (this.pos >= fence) {
                this.nextRunningState = 8;
                this.runningState = 12;
                break;
              }
            }
          }
          if (this.runningState === 8) this.runningState = 4;
          continue;
        case 9:
          {
            if (this.distance > 0x7FFFFFFC) {
              throw new BrotliDecoderDistanceError(
                "Invalid backward reference",
              );
            }
            const address = this.distance - this.maxDistance - 1 -
              this.cdTotalSize;
            if (address < 0) {
              this.initializeCompoundDictionaryCopy(
                -address - 1,
                this.copyLength,
              );
              this.runningState = 14;
            } else {
              const wordLength = this.copyLength;
              if (wordLength > 31) {
                throw new BrotliDecoderAllocRingBuffer1Error(
                  "Invalid backward reference",
                );
              }
              const shift = State.sizeBits[wordLength];
              if (shift === 0) {
                throw new BrotliDecoderAllocRingBuffer1Error(
                  "Invalid backward reference",
                );
              }
              let offset = State.offsets[wordLength];
              ...
              const len = transforms.transformDictionaryWord(
                this.ringBuffer,
                this.pos,
          ...
          continue;
        case 14:
          this.pos += this.copyFromCompoundDictionary(fence);
          if (this.pos >= fence) {
            this.nextRunningState = 14;
            this.runningState = 12;
            return;
          }
          this.runningState = 4;
        ...
        case 12:
          this.ringBufferBytesReady = this.ringBufferSize > this.pos
            ? this.pos
            : this.ringBufferSize;
          this.runningState = 13;
        case 13:
          if (this.writeRingBuffer() === 0) return;
          if (this.pos >= this.maxBackwardDistance) {
            this.maxDistance = this.maxBackwardDistance;
          }
          if (this.pos >= this.ringBufferSize) {
            if (this.pos > this.ringBufferSize) {
              ringBuffer.copyWithin(0, this.ringBufferSize, this.pos);
            }
            this.pos = this.pos & ringBufferMask;
    ...
    if (this.runningState === 10) {
      if (this.metaBlockLength < 0) {
        throw new BrotliDecoderBlockLength1Error("Invalid metablock length");
      }
  ...
  protected doReadMoreInput(): void {
    if (this.endOfStreamReached !== 0 && this.halfAvailable() < -2) {
      throw new BrotliDecoderNeedsMoreInputResult();
    }
    const readOffset = this.halfOffset << 1;
    let bytesInBuffer = 4096 - readOffset;
    this.byteBuffer.copyWithin(0, readOffset, 4096);
    this.halfOffset = 0;
    while (bytesInBuffer < 4096) {
      const spaceLeft = 4096 - bytesInBuffer;
      const len = this.readInput(this.byteBuffer, bytesInBuffer, spaceLeft);
      if (len <= 0) {
        this.endOfStreamReached = 1;
        this.tailBytes = bytesInBuffer;
        bytesInBuffer += 1;
        break;
      }
      bytesInBuffer += len;
    }
    this.bytesToNibbles(bytesInBuffer);
  }

  protected checkHealth(endOfStream: number): void {
    if (this.endOfStreamReached !== 0) {
      const byteOffset = (this.halfOffset << 1) +
        ((this.bitOffset + 7) >> 3) -
        4;
      if (byteOffset > this.tailBytes) {
        throw new BrotliDecoderNeedsMoreInputResult();
      }
      if ((endOfStream !== 0) && (byteOffset !== this.tailBytes)) {
        throw new BrotliDecoderNeedsMoreOutputResult();
      }
    }
  }

  protected readFewBits(n: number): number {
    const v = (this.accumulator32 >>> this.bitOffset) & ((1 << n) - 1);
    this.bitOffset += n;
    return v;
  }
  ...
  protected readManyBits(n: number): number {
    ...
    this.accumulator32 = (this.shortBuffer[this.halfOffset++] << 16) |
      (this.accumulator32 >>> 16);
    this.bitOffset -= 16;
    return low | (this.readFewBits(n - 16) << 16);
  }

  protected jumpToByteBoundary(): void {
    const padding = (32 - this.bitOffset) & 7;
    if (padding !== 0) {
      const paddingBits = this.readFewBits(padding);
      if (paddingBits !== 0) throw new BrotliDecoderPadding1Error();
    }
  }

  protected halfAvailable(): number {
    let limit = 2048;
    if (this.endOfStreamReached !== 0) limit = (this.tailBytes + 1) >> 1;
    return limit - this.halfOffset;
  }

  protected decodeWindowBits(): number {
    const isLargeWindow = this.isLargeWindow;
    ...
    if (this.readFewBits(1) === 0) return 16;
    let n = this.readFewBits(3);
    if (n !== 0) return 17 + n;
    n = this.readFewBits(3);
    if (n !== 0) {
      if (n === 1) {
        if (isLargeWindow === 0) return -1;
        this.isLargeWindow = 1;
        if (this.readFewBits(1) === 1) return -1;
        n = this.readFewBits(6);
  ...
  protected decodeVarLenUnsignedByte(): number {
    ...
    if (this.readFewBits(1) !== 0) {
      const n = this.readFewBits(3);
      return n === 0 ? 1 : this.readFewBits(n) + (1 << n);
    }
    return 0;
  }
  ...
  //     const sizeNibbles = this.readFewBits(2) + 4;
  //     if (sizeNibbles === 7) {
  //       this.isMetadata = 1;
  //       if (this.readFewBits(1) !== 0) throw new BrotliDecoderReservedError();
  //       const sizeBytes = this.readFewBits(2);
  ...
  //           this.bitOffset -= 16;
  //         }
  //         const bits = this.readFewBits(8);
  //         if (bits === 0 && i + 1 === sizeBytes && sizeBytes > 1) {
  //           throw new BrotliDecoderExuberantMetaNibbleError();
  //         }
  ...
  //           this.bitOffset -= 16;
  //         }
  //         const bits = this.readFewBits(4);
  //         if (bits === 0 && i + 1 === sizeNibbles && sizeNibbles > 4) {
  //           throw new BrotliDecoderExuberantNibbleError();
  //         }
  //         this.metaBlockLength += bits << (i * 4);
  //       }
  //     }
  //     this.metaBlockLength++;
  //     if (this.inputEnd === 0) this.isUncompressed = this.readFewBits(1);
  //   }
  // }

  protected decodeLiteralBlockSwitch(): void {
    this.literalBlockLength = this.decodeBlockTypeAndLength(
      0,
      this.numLiteralBlockTypes,
    );
    const literalBlockType = this.rings[5];
    this.contextMapSlice = literalBlockType << 6;
    this.literalTreeIdx = this.contextMap[this.contextMapSlice] & 0xFF;
    const contextMode = this.contextModes[literalBlockType];
    this.contextLookupOffset1 = contextMode << 9;
    this.contextLookupOffset2 = this.contextLookupOffset1 + 0x100;
  }
  ...
  protected decodeContextMap(
    ...
    if (this.halfOffset > 2030) this.doReadMoreInput();
    const numTrees = this.decodeVarLenUnsignedByte() + 1;
    if (numTrees === 1) {
    ...
    const useRleForZeros = this.readFewBits(1);
    let maxRunLengthPrefix = 0;
    if (useRleForZeros !== 0) maxRunLengthPrefix = this.readFewBits(4) + 1;
    ...
    while (i < contextMapSize) {
      ...
      const code = this.readSymbol(table, tableIdx);
      if (code === 0) {
        contextMap[i] = 0;
        i++;
        ...
        let reps = (1 << code) + this.readFewBits(code);
        while (reps !== 0) {
          if (i >= contextMapSize) {
            throw new BrotliDecoderContextMapRepeatError("Invalid context map");
          }
          contextMap[i] = 0;
          i++;
          reps--;
  ...
  protected decodeBlockTypeAndLength(
    ...
    let blockType = this.readSymbol(this.blockTrees, 2 * treeType);
    const result = this.readBlockLength(this.blockTrees, 2 * treeType + 1);
    if (blockType === 1) {
  ...
  protected inverseMoveToFrontTransform(v: Int8Array, vLen: number): void {
    ...
    for (let i = 0; i < vLen; ++i) {
      ...
      v[i] = mtf[index];
      if (index !== 0) this.moveToFront(mtf, index);
    }
  }

  protected maybeReallocateRingBuffer(): void {
    const REALLOCATION_SLACK = 37;

    let newSize = this.maxRingBufferSize;
    if (newSize > this.expectedTotalSize) {
      const minimalNewSize = this.expectedTotalSize;
      while ((newSize >> 1) > minimalNewSize) newSize >>= 1;
      if (
        (this.inputEnd === 0) && newSize < CHUNK_SIZE &&
        this.maxRingBufferSize >= CHUNK_SIZE
      ) newSize = CHUNK_SIZE;
    }
    if (newSize > this.ringBufferSize) {
      const newBuffer = new Int8Array(newSize + REALLOCATION_SLACK);
      if (this.ringBuffer.length !== 0) {
        newBuffer.set(this.ringBuffer.subarray(0, this.ringBufferSize), 0);
      }
  ...
  protected readHuffmanCodeLengths(
    ...
    let symbol = 0, prevLen = 8, repeat = 0, repeatLen = 0, space = 32768;
    ...
    while (symbol < numSymbols && space > 0) {
      if (this.halfOffset > 2030) this.doReadMoreInput();
      if (this.bitOffset >= 16) {
        const acc = this.shortBuffer[this.halfOffset++] << 16;
        this.accumulator32 = acc | (this.accumulator32 >>> 16);
        this.bitOffset -= 16;
      }
      const p = (this.accumulator32 >>> this.bitOffset) & 31;
      this.bitOffset += table[p] >> 16;
      ...
      if (codeLen < 16) {
        ...
        let newLen = 0;
        if (codeLen === 16) newLen = prevLen;
        if (repeatLen !== newLen) [repeat, repeatLen] = [0, newLen];
        ...
        if (symbol + delta > numSymbols) {
          throw new BrotliDecoderHuffmanSpaceError(
            "symbol + delta > numSymbols",
          );
        }
        for (let i = 0; i < delta; ++i) {
          codeLengths[symbol++] = repeatLen;
        }
        if (repeatLen !== 0) space -= delta << (15 - repeatLen);
      }
    }
    if (space !== 0) {
      throw new BrotliDecoderClSpaceError(`Unused space: ${space}`);
    }
  ...
  protected readNextMetablockHeader(): void {
    ...
    // begin inlined decodeMetaBlockLength (perf)
    if (this.bitOffset >= 16) {
      this.accumulator32 = (this.shortBuffer[this.halfOffset++] << 16) |
        (this.accumulator32 >>> 16);
      this.bitOffset -= 16;
    ...
    // end of inlined decodeMetaBlockLength

    if (this.metaBlockLength || this.isMetadata) {
      if (this.isUncompressed || this.isMetadata) {
        this.jumpToByteBoundary();
        this.runningState = this.isMetadata ? 5 : 6;
      } else {
        this.runningState = 3;
      }
      ...
      if (!this.isMetadata) {
        ...
        if (this.expectedTotalSize > 1 << 30) this.expectedTotalSize = 1 << 30;
  ...
  protected readMetablockPartition(
    ...
    let offset = this.blockTrees[2 * treeType];
    if (numBlockTypes <= 1) {
  ...
  protected readSimpleHuffmanCode(
    ...
    const numSymbols = this.readFewBits(2) + 1;
    for (let i = 0; i < numSymbols; ++i) {
      ...
      const symbol = this.readFewBits(maxBits);
      if (symbol >= alphabetSizeLimit) {
        throw new BrotliDecoderSimpleHuffmanAlphabetError(
          `Failed to read simple Huffman code symbol. ` +
            `Alphabet size limit: ${alphabetSizeLimit}, symbol: ${symbol}`,
        );
      }
      symbols[i] = symbol;
    }
    this.checkDupes(symbols, numSymbols);
    let histogramId = numSymbols;
    if (numSymbols === 4) histogramId += this.readFewBits(1);
    switch (histogramId) {
      case 1:
  ...
  protected readComplexHuffmanCode(
    ...
    for (let i = skip; i < 18; ++i) {
      ...
      const p = (this.accumulator32 >>> this.bitOffset) & 15;
      this.bitOffset += FIXED_TABLE[p] >> 16;
    ...
    if (space !== 0 && numCodes !== 1) {
      throw new BrotliDecoderHuffmanSpaceError(
        "Corrupted Huffman code histogram",
      );
    }

    this.readHuffmanCodeLengths(
  ...
  protected readHuffmanCode(
    ...
    const simpleCodeOrSkip = this.readFewBits(2);
    if (simpleCodeOrSkip === 1) {
      return this.readSimpleHuffmanCode(
        alphabetSizeMax,
        alphabetSizeLimit,
        tableGroup,
        tableIdx,
      ...
      return this.readComplexHuffmanCode(
        alphabetSizeLimit,
        simpleCodeOrSkip,
        tableGroup,
        tableIdx,
  ...
  protected readSymbol(
    tableGroup: Int32Array,
    tableIdx: number,
  ): number {
    let offset = tableGroup[tableIdx];
    const v = this.accumulator32 >>> this.bitOffset;
    offset += v & 0xFF;
    ...
    const mask = (1 << bits) - 1;
    offset += (v & mask) >>> 8;
    this.bitOffset += (tableGroup[offset] >> 16) + 8;
    return tableGroup[offset] & 0xFFFF;
  }

  protected readBlockLength(
    tableGroup: Int32Array,
    tableIdx: number,
  ): number {
    if (this.bitOffset >= 16) {
    ...
    const code = this.readSymbol(tableGroup, tableIdx);
    ...
    return BLOCK_LENGTH_OFFSET[code] +
      ((n <= 16) ? this.readFewBits(n) : this.readManyBits(n));
  }
  ...
  protected calculateDistanceAlphabetLimit(
    ...
    if (maxDistance < ndirect + (2 << npostfix)) {
      throw new BrotliDecoderDistanceError("Invalid distance alphabet size");
    }
  ...
  protected readMetablockHuffmanCodesAndContextMaps(): void {
    try {
      ...
      this.literalBlockLength = this.readMetablockPartition(
        0,
        this.numLiteralBlockTypes,
      );
    } catch (cause) {
      throw new BrotliDecoderAllocBlockTypeTreesError(
        "Failed to read literal block type trees",
        { cause } as { cause: Error },
      );
    }

    try {
      this.numCommandBlockTypes = this.decodeVarLenUnsignedByte() + 1;
      this.commandBlockLength = this.readMetablockPartition(
        1,
        this.numCommandBlockTypes,
      );
    } catch (cause) {
      throw new BrotliDecoderAllocBlockTypeTreesError(
        "Failed to read command block type trees",
        { cause } as { cause: Error },
      );
    }
    ...
    try {
      ...
      this.distanceBlockLength = this.readMetablockPartition(
        2,
        this.numDistanceBlockTypes,
      );
    } catch (cause) {
      throw new BrotliDecoderAllocBlockTypeTreesError(
        "Failed to read distance block type trees",
        { cause } as { cause: Error },
      );
    }
    ...
    try {
      ...
      const dpb = this.distancePostfixBits = this.readFewBits(2);
      this.numDirectDistanceCodes = this.readFewBits(4) << dpb;
      this.contextModes = new Int8Array(this.numLiteralBlockTypes);
      let i = 0;
      while (i < this.numLiteralBlockTypes) {
        let limit = i + 96;
        if (limit > this.numLiteralBlockTypes) {
          limit = this.numLiteralBlockTypes;
        }
        while (i < limit) {
          if (this.bitOffset >= 16) {
            this.accumulator32 = (this.shortBuffer[this.halfOffset++] << 16) |
          ...
          this.contextModes[i] = this.readFewBits(2);
          i++;
        }
        if (this.halfOffset > 2030) this.doReadMoreInput();
      }
    } catch (cause) {
      throw new BrotliDecoderAllocContextModesError(
        "Failed to read context modes",
        { cause } as { cause: Error },
      );
    }

    let numLiteralTrees = 0, numDistTrees = 0;

    const contextMapLength = this.numLiteralBlockTypes << 6;
    try {
      this.contextMap = new Int8Array(contextMapLength);
      numLiteralTrees = this.decodeContextMap(
        contextMapLength,
        this.contextMap,
      );
      ...
      for (let j = 0; j < contextMapLength; ++j) {
        if (this.contextMap[j] !== j >> 6) {
          this.trivialLiteralContext = 0;
          break;
        }
      }
    } catch (cause) {
      throw new BrotliDecoderAllocContextMapError(
        "Failed to read literal context map",
        { cause } as { cause: Error },
      );
    }

    try {
      this.distContextMap = new Int8Array(this.numDistanceBlockTypes << 2);
      numDistTrees = this.decodeContextMap(
        this.numDistanceBlockTypes << 2,
        this.distContextMap,
      );
    } catch (cause) {
      throw new BrotliDecoderAllocContextMapError(
        "Failed to read distance context map",
        { cause } as { cause: Error },
      );
    }

    try {
      this.literalTreeGroup = this.decodeHuffmanTreeGroup(
        256,
        256,
        numLiteralTrees,
      );

      this.commandTreeGroup = this.decodeHuffmanTreeGroup(
        704,
        704,
        this.numCommandBlockTypes,
      );

      let distanceAlphabetSizeMax = 16 + this.numDirectDistanceCodes +
        2 * (24 << this.distancePostfixBits);
      let distanceAlphabetSizeLimit = distanceAlphabetSizeMax;
      if (this.isLargeWindow === 1) {
        distanceAlphabetSizeMax = 16 + this.numDirectDistanceCodes +
          2 * (62 << this.distancePostfixBits);
        distanceAlphabetSizeLimit = this.calculateDistanceAlphabetLimit(
          0x7FFFFFFC,
      ...
      this.distanceTreeGroup = this.decodeHuffmanTreeGroup(
        distanceAlphabetSizeMax,
        distanceAlphabetSizeLimit,
        numDistTrees,
      );

      this.calculateDistanceLut(distanceAlphabetSizeLimit);
      this.contextMapSlice = this.distContextMapSlice = 0;
      this.contextLookupOffset1 = this.contextModes[0] * 512;
      this.contextLookupOffset2 = this.contextLookupOffset1 + 256;
      this.literalTreeIdx = this.commandTreeIdx = 0;
      this.rings[4] = this.rings[6] = this.rings[8] = 1;
      this.rings[5] = this.rings[7] = this.rings[9] = 0;
    } catch (cause) {
      throw new BrotliDecoderAllocTreeGroupsError(
        "Failed to read Huffman tree groups",
        { cause } as { cause: Error },
      );
    }
  }
    ...
    for (let i = 0; i < length - 1; ++i) {
      for (let j = i + 1; j < length; ++j) {
        if (symbols[i] === symbols[j]) {
          throw new BrotliDecoderSimpleHuffmanSameError(
            "Duplicate simple Huffman code symbol",
          );
        }
      }
    }
  ...
  protected writeRingBuffer(): number {
    const available = this.ringBufferBytesReady - this.ringBufferBytesWritten;
    let toWrite = this.outputLength - this.outputUsed;
    if (toWrite > available) toWrite = available;

    if (toWrite !== 0) {
      this.output.set(
        this.ringBuffer.subarray(
          this.ringBufferBytesWritten,
          this.ringBufferBytesWritten + toWrite,
        ),
        this.outputOffset + this.outputUsed,
      );
      this.outputUsed += toWrite;
      this.ringBufferBytesWritten += toWrite;
    }

    return this.outputUsed < this.outputLength ? 1 : 0;
  }
   ...
   * @internal
   */
  static get dictionaryData(): Int8Array {
    return State.#dictionaryData;
  }

  /** Contains the size bit offsets for each dictionary size. */
  static get offsets(): Int32Array {
    return State.#offsets;
  }

  /** Contains the size bits for each dictionary size. */
  static get sizeBits(): Int32Array {
    return State.#sizeBits;
  }

  /**
   * Contains a pre-built {@linkcode Transforms} instance with RFC 7932
   * transforms for the Brotli decompression algorithm, which are used to
   * transform the Huffman codes into the actual code values.
   */
  static get transforms(): Transforms {
    return State.#transforms; // initialized in the static block at the bottom
  }
  ...
  static setDictionaryData(newData: Int8Array, newSizeBits: Int32Array): void {
    if (newSizeBits.length > 31) {
      throw new BrotliDecoderDictionaryError(
        `New sizebits length must be < 32, got ${newSizeBits.length}`,
      );
    }
    for (let i = 0; i < 4; ++i) {
      if (newSizeBits[i] !== 0) {
        throw new BrotliDecoderDictionaryError(
          `Expected newSizeBits#${i} to be 0, got ${newSizeBits[i]}`,
        );
      }
    }
    State.sizeBits.set(newSizeBits.subarray(0, newSizeBits.length), 0);
    let pos = 0;
    const limit = newData.length;
    for (let i = 0; i < newSizeBits.length; ++i) {
      State.offsets[i] = pos;
      const bits = State.sizeBits[i];
      if (bits !== 0) {
        if (bits >= 31) {
          throw new BrotliDecoderDictionaryError(
            `Invalid bit length at index ${i}: ${bits} (expected 0 < bits < 31)`,
          );
        }
        pos += i << bits;
        if (pos <= 0 || pos > limit) {
    ...
    for (let i = newSizeBits.length; i < 32; ++i) State.offsets[i] = pos;
    if (pos !== limit) {
      throw new BrotliDecoderDictionaryError(
        "newSizeBits is inconsistent: underflow",
      );
    }
    State.#dictionaryData = newData;
  }
  ...
  static decompress(
    ...
    return new State(bytes, dict).decompress();
  }

  /**
   * Initializes the default dictionary data and size bits, which are used to
   * configure the decompression state when no custom dictionary is provided.
   *
   * This method is called automatically when the `State` class is first
   * loaded, and should not be called manually.
   * @internal
   */
  static initializeDefaultDictionary(): void {
    if (State.initialized) return;
    const dictionary = new Int8Array(122784), sizeBits = new Int32Array(25);
    const data0 =
    ...
    const sizeBitsData = "AAAAKKLLKKKKKJJIHHIHHGGFF";
    const s = data0 + data1;
    const dict = new Int8Array(s.length);

    for (let i = 0; i < s.length; ++i) {
      dict[i] = StringPrototypeCharCodeAt(s, i);
    }
    if (dict.length !== dictionary.length) {
      throw new BrotliDecoderDictionaryError(
        `Corrupted brotli dictionary data: expected ${dictionary.length} bytes, got ${dict.length}`,
      );
    }

    let offset = 0;
    const n = skipFlip.length >> 1;
    for (let i = 0; i < n; ++i) {
      const skip = StringPrototypeCharCodeAt(skipFlip, 2 * i) - 36;
      const flip = StringPrototypeCharCodeAt(skipFlip, 2 * i + 1) - 36;
      for (let j = 0; j < skip; ++j) dict[offset] = dict[offset++] ^ 3;
      for (let j = 0; j < flip; ++j) dict[offset] = dict[offset++] ^ 236;
    }
    for (let i = 0; i < sizeBitsData.length; ++i) {
      sizeBits[i] = StringPrototypeCharCodeAt(sizeBitsData, i) - 65;
    }
    dictionary.set(dict);
    State.setDictionaryData(dictionary, sizeBits);
  }
  // #endregion static methods

  // #region transforms initialization
  static {
    this.#transforms = new Transforms(121, 167, 50).unpack(
      `# #s #, #e #.# the #.com/#\xC2\xA0# of # and # in # to #"#">#\x0A#]# for # a # that #. # with #'# from # by #. The # on # as # is #ing #\x0A\x09#:#ed #(# at #ly #="# of the #. This #,# not #er #al #='#ful #ive #less #est #ize #ous #`,
      `     !! ! ,  *!  &!  " !  ) *   * -  ! # !  #!*!  +  ,$ !  -  %  .  / #   0  1 .  "   2  3!*   4%  ! # /   5  6  7  8 0  1 &   $   9 +   :  ;  < '  !=  >  ?! 4  @ 4  2  &   A *# (   B  C& ) %  ) !*# *-% A +! *.  D! %'  & E *6  F  G% ! *A *%  H! D  I!+!  J!+   K +- *4! A  L!*4  M  N +6  O!*% +.! K *G  P +%(  ! G *D +D  Q +# *K!*G!+D!+# +G +A +4!+% +K!+4!*D!+K!*K`,
    );
  }
  // #endregion transforms initialization
}
